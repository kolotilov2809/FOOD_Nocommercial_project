...
//переменная, которая определяет дедлайн
    const deadline = '2022-06-11';//поместим дату в виде строки
...
/* 	fetch('http://localhost:3000/menu')
   //fetch('db.json')
    ...
    let slideIndex = 1;//индекс для определения текущего положения в слайдере. поставили 1, потому что информацию будем выводить в человекоподобном варианте, когда первый слайд является 1. Поэтому использовали 1. В будущем это предусмотреть
...
/*     let sex = 'female',//значение по умолчанию женский пол
    ...
    if (localStorage.getItem('sex')) {//если в localStorage сохранен пол
        sex = localStorage.getItem('sex');//то пол присвоить значение из localStorage
    } else {//иначе
        sex = 'female';//полу присваеиваем значение женский
        localStorage.setItem('sex', 'female');//в localStorage сохранить пол женский
    }

    if (localStorage.getItem('ratio')) {//логика такая же , как и с полом
    ...
    //инициализирует наш калькулятор, когда заходим на страницу, она 1 раз срабатывает, устанавлиает классы в нужные нам блоки и после этого не будет работать
    function initLocalSettings(selector, activeClass) {//принимает селектор и активкласс
        const elements = document.querySelectorAll(selector);//получаем элементы, с которыми будем работать

        elements.forEach(elem => {//берем элементы, перебираем каждый отдельно элемент
            elem.classList.remove(activeClass);//у элемента удалим класс активности
            if (elem.getAttribute('id') === localStorage.getItem('sex')) {//если у перебираемого элемента прямо сейчас, если атрибут у элемента строго равен тому значению, что лежит в localStorage (по id  проверяем пол)
                elem.classList.add(activeClass);//то назначаем данному элементу(div) класс активности
            }
            if (elem.getAttribute('data-ratio') === localStorage.getItem('ratio')) {//такая же логика , как у пола выше
                elem.classList.add(activeClass);
            }
        });
    }
//применение функции для сравнения данных из localStorage
        ...
        elements.forEach(elem => {//перебираем все элементы внутри
            elem.addEventListener('click', (e) => {//для каждого элема навешиваем событие
                if (e.target.getAttribute('data-ratio')) {//если объект события (e.target) получает атрибут data-ratio
                    ...
                    localStorage.setItem('ratio', +e.target.getAttribute('data-ratio'));//используем localStorage, чтобы сохранить активность
                } else {//для работы с полом
                    sex = e.target.getAttribute('id');//получаем его по id
                    localStorage.setItem('sex', e.target.getAttribute('id'));//используем localStorage, чтобы сохранить пол
                }
    ...
    getStaticInformation('#gender div', 'calculating__choose-item_active');//#gender - id пола, calculating__choose-item_active - класс активности
    getStaticInformation('.calculating__choose_big div', 'calculating__choose-item_active');//.calculating__choose_big - получение родителя активности, calculating__choose-item_active - класс активности
    ...
    function getDynamicInformation(selector) {//принимает только один аргумент, selector того инпута, котрый нас интересует
        ...
        input.addEventListener('input', () => {//навешиваем обработчик события на инпут
            if (input.value.match(/\D/g)) {//если в input не цифры
                input.style.border = '1px solid red';//применить обводку инпута красной сплошной линией в 1px
            } else {//иначе
                input.style.border = 'none';//не применять 
            }
