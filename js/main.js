...
window.addEventListener('DOMContentLoaded', () => {
    ...
    let tabsContent = document.querySelectorAll('.tabcontent');   //весь контент, который находится в верстке
    let tabsParent = document.querySelector('.tabheader__items');   //родитель, который будет содержать все табы
    ...
    const deadline = '2021-12-31';//поместим дату в виде строки

    //функция которая определяет разницу между дедлайном и текущим временем
    function getTimeRemaining(endtime) {
        //в переменную t  положим результат. Date.parse(endtime) - колво миллисекунд, которое будет в нашем конечном времени. Date.parse(new Date()) - текущее время
        const t = Date.parse(endtime) - Date.parse(new Date()),//когда функция запустится. мы получим разницу между
    ...
    function setClock(selector, endtime) {//нам необходим блок, куда вставим таймер class="timer", обозначаем selector. а так же сам дедлайн, который будем ему передавать endtime
        ...
        const timer = document.querySelector(selector),
            ...
            //обновление времени. запускаем функцию updateClock через каждую 1 секунду
            timeInterval = setInterval(updateClock, 1000);


        //Чтобы не было мигания на странице (отображаются в таймере цифры, которые стоят в верстке). Это происходит потому что, наша функция updateClock запускается с интервалом в 1 секунду
        updateClock();//поэтому мы вручную запускаем updateClock. это такая функция инициализации, которая один раз запустится, установит текущую дату и потом она исчезнет и будет работать setInterval
        ...
        function updateClock() {
            ...
            //чтобы остановить таймер. Если время уже вышло (в отрицательную сторону или равно 0)
            if (t.total <= 0) {
    ...
    const modal = document.querySelector('.modal');//отвечает за модальное окно
    ...
    //используем метод forEach, то что внутри будет какая то кнопка
    modalTrigger.forEach(btn => {
        //когда кликаем по кнопке для вызова модал окна
        btn.addEventListener('click', openModal);
    });


    //функция для закрытия модал окна
    function closeModal() {
    ...
    //Чтобы закрыть модал окно по клику в любую область на странице. на наше модальное окно навешиваем обработчик клика
    ...
    //чтобы модал окно появлялось по прошествии времени
    const modalTimerId = setTimeout(openModal, 50000);

    //функция для открытия модал окна, если польз-ль долистал до конца страницы
    ...
    //если пользователь долистал до конца страницы, модал окно должно открыться
    ...
    class MenuCard {//создадим шаблон класс, чтобы от него отпочковывать карточки
        ...
        //метод для конвертации валют из  $ в грв
        changeToUAH() {
        ...
        //метод, чтобы сформировать верстку. Здесь нам нужно создать element, в него поместить определенную верстку, эту верстку дополнить данными, которые приходят как аргументы. и поместить этот элемент уже на страницу
        render() {
...
/*     getResource('http://localhost:3000/menu')
    ...
    //формы. Формы у нас 2, поэтому функционал по отправке будет повторяться. Для этого мы обернем в функцию, которую будем вызывать при отправке формы

    //получим все формы, которые есть на странице
    const forms = document.querySelectorAll('form');

    //создадим объект для вывода сообщения. что у нас произошло при отправке данных
    const message = {
    ...
    const postData = async (url, data) => {//async внутри функции будет асинхронный код
        ...
        const res = await fetch(url, {//await ставится перед теми операциями, которые надо дождаться
        ...
        return await res.json();//async и await всегда работают в паре
    };
    ...
    function bindPostData(form) {
        form.addEventListener('submit', (e) => {
            ...
            const statusMessage = document.createElement('img');//создаем img тэг
...
<<<<<<< HEAD
    ...
    for (let i = 0; i < slides.length; i++) {
        ...
        dot.setAttribute('data-slaide-to', i + 1);//каждой точке будем устанавливать атрибут data-slaide-to и будем ставить нумерацию , начиная с 1
        dot.style.cssText = `
        ...
        `;
    //для отображнния класса активности
    if (i == 0) {
        dot.style.opacity = 1;
    }
    ...
    next.addEventListener('click', () => {//при клике запускаем колбэк функцию
        if (offset == (+width.slice(0, width.length - 2) * (slides.length - 1))) {//если наш отступ будет равен ширине одного слайда умноженное на колво слайдов -1, то в таком случае
			offset = 0;//устанавливаем offset в 0. Это значит что долистали до самого конца и нужно вернуться в самое начало
                ...
                offset += +width.slice(0, width.length - 2); //то добавляем смещение
            }
    ...
    prev.addEventListener('click', () => {
        if (offset == 0) {//проверяем первый слайд
                offset = +width.slice(0, width.length - 2) * (slides.length - 1);//то в переменную offset записываем последний слайд по формуле 
			} else {//а если был не первый слайд
               offset -= +width.slice(0, width.length - 2);//то отнимаем значение
            }
...
>>>>>>> 1e38cfe66315d4fa8f09bcd3c8bd82df8cdc63ec
